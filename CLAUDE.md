...
```bash
pnpm test                      # Backend unit tests (79 tests, no Docker needed)
pnpm test:integration          # Backend integration tests (50 tests, needs Docker for Testcontainers)
cd web && pnpm test            # Frontend component tests (36 tests, no Docker needed)
pnpm test:e2e                  # E2E Playwright tests (14 tests, needs Docker postgres; stop the api container so Playwright's dev server gets port 3001 with MOCK_AGENT)
```
...
thesis-tracking/
  ...
  src/
    ...
    services/
      thesis-generation.ts     — Orchestrates: validate → agent call → persist thesis
      weekly-monitoring.ts     — Weekly monitoring: market data → agent analysis → persist log + getCurrentWeek() export
...
| S8 | 2 | Jun 2-6 | Email digest + polish (digest later removed in spec 07) | Done |
